...
\makeatother
%\makeatletter
%\renewcommand\thesubfigure{\@arabic\c@chapter--\@arabic\c@subfigure}%
%\makeatother
\makeatletter
\renewcommand\thetable{\@arabic\c@chapter-\@arabic\c@table}%
\makeatother
\titlecontents{chapter}[0pt]{\zihao{-4}\selectfont\heiti\addvspace{2pt}\filright}
              {\contentspush{\thecontentslabel\ }}
              {}{\titlerule*[8pt]{.}\contentspage}
\titlecontents{section}[8mm]{\zihao{-4}\selectfont\addvspace{2pt}\filright}
              {\contentspush{\thecontentslabel\ }}
              {}{\titlerule*[8pt]{.}\contentspage}
\titlecontents{subsection}[16mm]{\zihao{-4}\selectfont\addvspace{2pt}\filright}
              {\contentspush{\thecontentslabel\ }}
              {}{\titlerule*[8pt]{.}\contentspage}
\captionsetup[figure]{name=\heiti 图,labelsep=space}
\captionsetup[table]{name=\heiti 表,labelsep=space}
\titleformat{\chapter}{\vspace{-2cm}\centering\zihao{3}\heiti}{第\,\chinese{chapter}\,章}{1em}{}[\vspace{-0.3cm}]
\titleformat{\section}{\zihao{-3}\heiti}{\chinese{section}}{1em}{}
\titleformat{\subsection}{\zihao{4}\heiti}{(\,\chinese{subsection}\,)}{1em}{}
\titleformat{\subsubsection}{\zihao{-4}\heiti}{\arabic{subsubsection}}{1em}{}
...
\begin{document}
%%%%%%%-------------------------------------------------
\fenleihao{F000.0}  % 分类号:《中国图书资料分类法》的类号. 必填. 要根据自己的学科方向填写!!
\miji{}                % 密级
\bianhao{10475}  % 学校编号, 10475是河南大学的编号. 不用改动.

\title{河南大学硕士研究生毕业论文 \LaTeX 模板}
\Etitle{\LaTeX Template of Henan University} % 英文题目
\author{姓名}   %姓名
\StudentNumber{104750000000}   % 学号
\Eauthor{English name}    %作者英文名
\Ckind{XX学硕士}    %申请学位类别
\Csupervisor{XXX\quad 教授}        %指导教师中文名、职称
\Esupervisor{Prof. XXXX}     %指导教师英文名、职称
\Cmajor{XXXX学}                          % 专业中文名[ SMD:专业类别（领域）]
\Emajor{Major}% 专业英文名[ SMD:专业类别（领域）]
\Cspeciality{研究方向}                     % 研究方向
\Especiality{xxxx }   % 研究方向
\Schoolname{School of Economics} %学院英文名. 不确定的话, 请看一下自己学院的网页上是怎么写的. 别搞错了!
\date{二〇一七\ 年\ 六 \ 月}                    % 硕士类只写年月. 要注意和英文日期一致!!
\Edate{June, 2017}                   % 英文封面日期
...
\section{具体使用步骤}
\par 本模板根据武汉大学黄正华老师的武汉大学硕士论文\LaTeX 模板修改而来，已经完全符合河南大学对硕士学位论文的格式要求。
\begin{description}
...
\begin{tabular}{ll}
\hline
   \verb|\documentclass{HENUMaster}|                 &  硕士论文 \\
   \verb|\documentclass[forprint]{HENUMaster}|    &  硕士论文打印版  \\
   \verb|\documentclass[forlib]{HENUMaster}|       &  硕士论文图书馆提交版  \\
\hline
\end{tabular}
\end{table}
...
\begin{tabular}{ll}
\hline
   \verb|\documentclass[smd]{HENUMaster}|               &  专业硕士论文 \\
   \verb|\documentclass[smd,forprint]{HENUMaster}|    &  专业硕士论文打印版  \\
   \verb|\documentclass[smd,forlib]{HENUMaster}|       &  专业硕士论文图书馆提交版  \\
\hline
\end{tabular}
\end{table}
...
向河南大学图书馆提交电子文档, 需单独编译文件: 在文档选项中添加 forlib.
...
\hline
  \multicolumn{2}{l|}{HENUMaster.cls} &  定义文档格式的 class file. 不可删除.\\ \hline \hline
\end{tabular}
\end{table}
%  \footnotetext[1]{定制这个参考文献格式, 主要是希望尽可能满足河南大学硕士论文的相应格式要求, 比如要求将~article 的``年份''写在``卷号''之前.
...
封面上毛体字的``河南大学''也是这样做出的.



2016 年 02 月更新: 调整为适应 TeX Live 2015 的版本.

2016 年 04 月更新: 按照研究生院新发布的要求进行改造. 主要变化: (1) 封面``河南大学''字样改为毛体题字;
...
用形如 \verb|\includegraphics[width=12cm]{1.jpg}| 的命令可以纳入图片.

如图 \ref{fig:1} 是一个纳入~jpg 图片的例子.

\begin{figure}[ht]
\centering
  \includegraphics[width=\textwidth]{HENU.jpg}
...
\begin{figure}[!htbp]
\centering
\subfigure[\heiti\zihao{-5}1]{
\label{Fig.sub.1}
\includegraphics[width=5cm]{HENU.jpg}}
\subfigure[\heiti\zihao{-5}1]{
\label{Fig.sub.2}
\includegraphics[width=5cm]{HENU.jpg}}
\\
...
\label{Fig.sub.3}
\includegraphics[width=5cm]{HENU.jpg}}
\subfigure[\heiti\zihao{-5}11]{
\label{Fig.sub.4}
\includegraphics[width=5cm]{HENU.jpg}}
\caption{\heiti\zihao{-5}1}
\label{fig:2}
\end{figure}
...
%
%《河南大学博、硕士研究生学位论文缴送办法》要求``电子版应采用~Word97 或~Word2000 格式(DOC)编辑''\footnote{那已经是~2005 年的事情了.}.
%关于这个重要的问题, 我已致信河南大学图书馆总馆信息服务中心袁晓川老师,
...
\backmatter
\include{includefile/backmatter} %%%致谢, 河南大学学位论文使用授权协议书．
\cleardoublepage
\end{document}
